...
BEGIN
  ...
  -- CHANGED: We only count NON-SYSTEM messages to check if the game has "started" - so system messages like "joined" don't lock the game
  IF EXISTS (SELECT 1 FROM messages WHERE game_id = NEW.game_id AND id != NEW.id AND type != 'system' LIMIT 1) THEN
      IF v_current_turn_user_id IS NOT NULL AND v_current_turn_user_id != NEW.user_id THEN
        RAISE EXCEPTION 'It is not your turn!';
      END IF;
  END IF;

  -- 3. Calculate Turn Rotation (RPC usually does this, but Trigger is a safety fallback / auto-rotator for direct inserts if any)
