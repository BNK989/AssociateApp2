...
BEGIN
  ...
  IF v_game_status NOT IN ('lobby', 'texting', 'active') THEN
     -- If solving, we might allow it if it's a solve attempt? 
     -- But this RPC seems primarily for "Chatting/sending words".
     -- For solve attempts, we likely use a different flow or this one adapted?
     -- Assuming this is for "New Clue Messages".
