...
**This page outranks the experiment.** As long as a policy has been saved here
...
actually get — trust it over your own arithmetic. Beside it, pinned so it stays
in view while you edit, is a demo game you can play against the same numbers
(§3a).

Under **First word only** the chain has two different timelines — the first word
opens hinted, everything after it climbs from nothing — so the panel draws both.
One track means every word opens the same way.
...
Pinned beside the panels is **Try it**: a throwaway four-word chain, played
against whatever is in the editor right now — *including changes you have not
saved*.

It is the board rather than a sketch of it. The shipped composer with its letter
pool, the shipped stuck offer above it, the shipped drip underneath and the
shipped reward burst on a solve — so it is driven by **all four** policies on
this page, not only the hint ladder:

| Editing | What the demo shows you |
| :--- | :--- |
| The hint ladder (§2) | What each rung actually gives away, and when |
| Reward feedback | The burst, the sparks and the chime on a real solve |
| The letter pool | The halo, the caret and how letters land |
| The drip and the fork ([`settle_drip.md`](settle_drip.md)) | The offer where you put it, with the options you chose, and letters walking into place at the pace you set |
...
The controls above the board are the game master's, and a player has none of
them:

- **Fill the answer** types the current word in for you. You are testing hints,
  not solving a puzzle.
- **Hint** forces the next rung. It and the countdown behave exactly as they do
  in the daily game, because they are the same modules: the board build, the
  arrival pass, the ladder, the scheduler and the scoring are all the game's own.
- **Skip 10s ahead** credits the stuck ladder with dwell, the way a wrong guess
  does. The offer is on a twenty-second clock, so one press reaches the first
  offer and two reach the fork — you do not have to sit in silence to see the
  thing you just composed. The credit is per word and falls away with it.
- **Give up** retires the word for nothing, which is also what the reveal at the
  bottom of the ladder does.
- The score is real, so anything that charges is visible here: solve the same
  word with **Charge for the free hint** on and off, or with the drip's cost per
  letter high and low, and compare.
...
7. **The timeline preview runs the real scheduler.** Do not re-derive the timings
   for display — a preview that can drift is worse than none. Drawing only
   *one* of the chain's timelines is the same failure by omission: if a policy
   makes some words open differently, every version has to be on screen.
...
11. **The demo mounts the shipped components, not lookalikes of them.** The
   ...
   that costs. Only the harness around it — `DemoControls` — is demo-only.
