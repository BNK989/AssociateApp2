...
/**
 * Weight separates a letter from filler without relying on hue, so the two stay
 * distinguishable when colour is unavailable — a colour-blind player, a failed
 * web font, a screenshot run through a filter.
 */
...
export function tileClassName(state: TileState): string {
    const base = `${TILE_COLOR[state]} ${TILE_WEIGHT[state]}`;
